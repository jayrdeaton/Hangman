...
export type AutoSaveCustomContextType = {
  // Whether a word you author gets kept in the "Pass & Play" pack.
  //
  ...
  // composing or play — so a guesser who knows to look (Choose packs → Pass & Play → ✎) could in
  ...
  // anything in the Pass & Play pack is readable in plain text via Choose packs → Pass & Play → ✎,
  // and once that pack is selected the word joins the random draw pool. That's fine for a word you
  // wrote for yourself and surprising for one you wrote for someone sitting next to you, so the safe
  // default wins and PnpWordPrompt offers a toggle for the other case.
  autoSave: boolean
